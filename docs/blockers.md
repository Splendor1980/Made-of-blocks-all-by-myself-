...
- **One-click fix:** `setup-windows.bat` (repo root) runs `npm install` and then **self-heals** Electron: if `electron.exe`/`path.txt` are missing it auto-extracts a dropped `electron-v*-win32-x64.zip`, and it always (re)creates `path.txt`. `launch-skin-studio.bat` also recreates `path.txt` on every launch if the binary is present. So the only manual step left is dropping the zip next to the installer on machines where GitHub/`npm install` is blocked.
